...
export class RaptorQFECDecoder {
   ...
   * @returns {Uint8Array|null} Recovered data if decoding completed, null otherwise
   */
  tryDecode(streamId) {
    const stream = this.decoders.get(streamId);
    if (!stream) return null;
    if (stream.blockComplete) {
      return stream.recoveredData || null;
    }
    ...
    // If decoding is complete, get the recovered data
    if (stream.blockComplete) {
      try {
        const result = decoder.get_result();
        stream.recoveredData = new Uint8Array(result);
        this.stats.packetsRecovered++;
        return stream.recoveredData;
      } catch (e) {
        console.error('RaptorQ get_result error:', e);
      }
    }
  ...
  /**
   * Get recovered data for a stream (after decoding is complete)
   * @param {number} streamId - Stream identifier
   * @returns {Uint8Array|null} Recovered data or null
   */
  getRecoveredData(streamId) {
    const stream = this.decoders.get(streamId);
    if (!stream || !stream.blockComplete) return null;
    return stream.recoveredData || null;
  }
