 ...
 *
 * @param decoder   Decoder handle
 * @param out_data  Output buffer (must be at least transfer_length bytes)
 * @param out_len   On input: buffer size. On output: bytes written
 *
 * @return 0 on success, -1 on error (not complete or buffer too small)
 */
int raptorq_decoder_get_result(
