    ...
    try {
        ...
        if (isComplete) {
            ...
            logTest(
                'Data Integrity',
                matches,
                `Original: ${data.length} bytes, Decoded: ${decoded.length} bytes`
            );
...
function testFECRecovery(wasm) {
    ...
    const testCases = [
        { lossRate: 0.10, name: '10% loss', repairSymbols: 3 },
        { lossRate: 0.20, name: '20% loss', repairSymbols: 5 },
        { lossRate: 0.30, name: '30% loss', repairSymbols: 8 }
    ];
...
function testPerformance(wasm) {
    ...
    try {
        ...
        for (let i = 0; i < iterations; i++) {
            lastEncoder = new wasm.RaptorQEncoder(
                data,
                1280,
                4,
                1,
