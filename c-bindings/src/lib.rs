...
use std::ptr;
use std::slice;
use raptorq::{Encoder, Decoder, SourceBlockEncoder, ObjectTransmissionInformation, EncodingPacket};
...
/// Get the decoded result data
/// Only valid after is_complete() returns true
///
/// # Arguments
/// * `decoder` - Decoder handle
/// * `out_data` - Output buffer (must be at least transfer_length bytes)
/// * `out_len` - On input: buffer size. On output: bytes written
///
/// # Returns
/// 0 on success, -1 on error (not complete or buffer too small)
#[no_mangle]
pub extern "C" fn raptorq_decoder_get_result(
