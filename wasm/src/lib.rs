...
impl RaptorQDecoder {
    ...
    pub fn add_packet(&mut self, packet: &[u8]) -> Result<bool, JsValue> {
        ...
        // Try to decode - returns Some if a block was just completed
        if self.decoder.decode(encoding_packet).is_some() {
    ...
    pub fn get_result(&self) -> Result<Vec<u8>, JsValue> {
        ...
        // The decoder stores the result internally after decode() returns Some
        // We need to reconstruct it from blocks
        Err(JsValue::from_str("Use try_decode for streaming decoding"))
    }
